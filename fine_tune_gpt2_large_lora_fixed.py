#!/usr/bin/env python3
"""
GPT-2 Large Medical AI Fine-tuning Script with LoRA (Fixed)

This script fine-tunes GPT-2 Large using LoRA (Low-Rank Adaptation) on your medical research database.
Fixed version that handles tokenization properly.
"""
...
class GPT2LargeLoRAFineTuner:
    ...
    def create_dataset(self, texts: List[str], tokenizer, max_length: int = 512):
        """Create a dataset for training with proper tokenization."""
        def tokenize_function(examples):
            # Tokenize the texts with proper padding
            tokenized = tokenizer(
                examples["text"],
                truncation=True,
    ...
    def setup_lora_config(self):
        """Setup LoRA configuration for GPT-2 Large."""
    ...
    def fine_tune(self, model_name: str, max_samples: int = 800):
        """Fine-tune the model using LoRA."""
        logger.info(f"Starting GPT-2 Large LoRA fine-tuning...")
        ...
        try:
            ...
            # Create dataset
            dataset = self.create_dataset(training_texts, tokenizer, max_length=512)
            
            # Training arguments optimized for LoRA
            training_args = TrainingArguments(
                output_dir="./gpt2-large-medical-lora",
                num_train_epochs=3,  # More epochs for LoRA
                per_device_train_batch_size=4,  # Larger batch size for GPT-2
            ...
            trainer.save_model()
            tokenizer.save_pretrained("./gpt2-large-medical-lora")
            
            logger.info("LoRA fine-tuning completed! Model saved to ./gpt2-large-medical-lora")
...
def main():
    """Main function to fine-tune GPT-2 Large with LoRA."""
    logger.info("🏥 GPT-2 Large Medical AI LoRA Fine-tuning")
    logger.info("=" * 60)
    ...
    if device == "cpu":
        logger.warning("⚠️ CPU detected. GPT-2 Large will be slow on CPU.")
    elif device == "mps":
        logger.info("🍎 Apple Silicon detected. Using MPS acceleration.")
        logger.info("GPT-2 Large with LoRA should work well with your 96GB RAM.")
    else:
        logger.info("🚀 CUDA detected. Using GPU acceleration.")
    
    # Use GPT-2 Large model
    model_name = "gpt2-large"
    logger.info(f"Selected model: {model_name}")
    
    # Initialize fine-tuner
    fine_tuner = GPT2LargeLoRAFineTuner()
    
    # Determine training parameters
    max_samples = 800  # Use 800 samples for LoRA training
    ...
    if success:
        ...
        logger.info("Your GPT-2 Large medical AI model is ready!")
        logger.info("Model saved to: ./gpt2-large-medical-lora/")
        ...
        try:
            ...
            generator = pipeline(
                "text-generation",
                model="./gpt2-large-medical-lora",
                tokenizer="./gpt2-large-medical-lora",
